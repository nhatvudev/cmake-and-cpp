...
function do_build() {
    ...
    if [ "$1" == "debug" ] ||  [ "$1" == "DEBUG" ]; then
        ...
        # cmake -DCMAKE_BUILD_TYPE=DEBUG -DBUILD_SHARED_LIBS=OFF -DCMAKE_VERBOSE_MAKEFILE:BOOL=ON ..\
        cmake -DBUILD_DIR="$BUILD_DIR" -DCMAKE_BUILD_TYPE=DEBUG -DBUILD_SHARED_LIBS=OFF -DCMAKE_VERBOSE_MAKEFILE:BOOL=ON "$CURR_DIR"
    else
        echo "build for RELEASE"
        # cmake -DBUILD_SHARED_LIBS=OFF -DCMAKE_VERBOSE_MAKEFILE:BOOL=ON ..
        cmake -DBUILD_DIR="$BUILD_DIR" -DBUILD_SHARED_LIBS=OFF -DCMAKE_VERBOSE_MAKEFILE:BOOL=ON "$CURR_DIR"
    fi
    # make $2
